...
- [PR brief](./pr-brief/) shows changed-area detection, noisy-PR warnings, reviewer focus, and the release note snippet.
- [Release draft](./release-draft/) shows the markdown and JSON artifacts used to build a draft GitHub release, including data quality notes when metadata is sparse.
- [Repository intelligence](./repo-intel/) shows the maintainer summary, repository shape, key entry paths, setup path, and major areas.
- [Onboarding map](./onboarding-map/) shows the reading order, setup checkpoints, contributor tracks, starter tasks, and escalation notes for new contributors.
...
omc summarize-pr --input tests/fixtures/pulls/noisy_multi_area_pr.json --output examples/pr-brief/output.json --markdown examples/pr-brief/output.md
...
omc onboarding-map --input tests/fixtures/repos/repo_intel_python_toolkit.json --output examples/onboarding-map/output.json --markdown examples/onboarding-map/output.md
```

The release example in this repository uses the March 20, 2026 render date so the checked-in output stays stable.
The workflow skills for these examples live in `skills/`.
